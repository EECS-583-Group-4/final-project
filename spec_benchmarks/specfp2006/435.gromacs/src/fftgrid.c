/*
 * $Id: fftgrid.c,v 1.1 2007/01/25 17:23:14 mchu Exp $
 * 
 ...
 */
static char *SRCID_fftgrid_c = "$Id: fftgrid.c,v 1.1 2007/01/25 17:23:14 mchu Exp $";
#include "assert.h"
#include "typedefs.h"
#include "futil.h"
#include "smalloc.h"
		      ...
		      t_complex ***grid)
{
  ...
  for(ix=0; (ix<nx); ix++)
    for(iy=0; (iy<ny); iy++)
      for(iz=0; (iz<nz); iz++) {
	g=grid[ix][iy][iz];
	if (fp  && (fabs(g.re)>GMX_REAL_MIN || fabs(g.im)>GMX_REAL_MIN))
