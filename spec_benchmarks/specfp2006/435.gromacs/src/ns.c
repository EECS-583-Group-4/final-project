/*
 * $Id: ns.c,v 1.1 2007/01/25 17:23:14 mchu Exp $
 * 
 ...
 */
static char *SRCID_ns_c = "$Id: ns.c,v 1.1 2007/01/25 17:23:14 mchu Exp $";
#ifdef USE_THREADS
			  ...
			  t_excl bexcl[],t_forcerec *fr)
{
  ...
  if (bBox) {
    ...
    for(j=0; (j<njcg); j++) {
      cg_j   = jcg[j];
      nrj    = cgindex[cg_j+1]-cgindex[cg_j];
      if ((rcut2 < GMX_REAL_MIN) || (distance2(x[icg],x[cg_j]) < rcut2)) {
	/* jgid  = gid[cga[cgindex[cg_j]]]; */
	jgid  = gid[cgindex[cg_j]];
	if (!i_eg_excl[jgid]) {
		    ...
		    bool bHaveLJ[])
{
  ...
  bool    *i_eg_excl;
  bool    use_twinrange,use_two_cutoffs;
			  ...
			  if (!i_eg_excl[jgid]) {
				...
				nlr_ljc[jgid]=0;
			      ...
			      nl_lr_ljc[jgid][nlr_ljc[jgid]++]=jjcg;
                              /* MSVC7 seems to have problems when rm2=rl2, 
			       * so I added this extra check to make sure we 
			       * have the nl_lr_one list before using it.
                               */
...
static int  rv_comp(const void *a,const void *b)
{
  ...
  if (diff < 0)
    return -1;
  else if (fabs(diff) < GMX_REAL_MIN)
    return 0;
  else
    return 1;
}
		      ...
		      real lambda,real *dvdlambda)
{
  ...
  if (bFirst) {
    ...
    for(i=0; (i<fr->ntype); i++) 
    {
	    ...
	    {
	      bHaveLJ[i] = bHaveLJ[i] ||
		( fabs(BHAMA(fr->nbfp,fr->ntype,i,j)) > GMX_REAL_MIN ) ||
                ( fabs(BHAMB(fr->nbfp,fr->ntype,i,j)) > GMX_REAL_MIN ) ||
                ( fabs(BHAMC(fr->nbfp,fr->ntype,i,j)) > GMX_REAL_MIN );
	    }
	    else
	    {
              bHaveLJ[i] = bHaveLJ[i] ||
                ( fabs(C6(fr->nbfp,fr->ntype,i,j)) > GMX_REAL_MIN ) ||
                ( fabs(C12(fr->nbfp,fr->ntype,i,j)) > GMX_REAL_MIN );
	    }
	}
    }
    if (debug) 
